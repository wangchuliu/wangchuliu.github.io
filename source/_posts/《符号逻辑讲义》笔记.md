...
## <span class="indianred">第二章</span> 命题联结词与真值表方法
...
## <span class="indianred">第三章</span> 命题逻辑的基本概念
### <span class="indianred">3.1</span> 对象语言里的符号和公式

{TODO: 符号的选择是任意的，我们关注的是对象语言的意义。语言学？}

### <span class="indianred">3.2</span> 真值指派和公式的真值

我们把 $\mathscr{L}_0$ 的所有命题变号的集合记作 $\textbf{Pr}$，因此 $\textbf{Pr}$ 是一个无穷集。一个**真值指派**（或**赋值**）是从 $\textbf{Pr}$ 到 $\{\T,\F\}$ 的函数 $\sigma$，它对每个命题变号 $p$ 指派一个真值 $\sigma(p)$。由于定义域是无穷集，真值指派也有无穷多个。

**（真理定义）**：对所有的 $\mathscr{L}_0$-公式，我们用 $\phi^\sigma$ 表示 <span class="underline">$\phi$ 在 $\sigma$ 下的值</span>。$\phi^\sigma$ 递归地定义如下：
$$\begin{aligned}
  p_n^\sigma = \T                         &\quad\text{iff}\quad    \sigma(p_n) = \T\ (n \geqslant 0)\\
  (\lnot\psi)^\sigma = \T                 &\quad\text{iff}\quad    \psi^\sigma \neq \T\ (\text{即 } \psi^\sigma = \F)\\
  (\psi \lor \chi)^\sigma = \T            &\quad\text{iff}\quad    \text{或者 }\psi^\sigma = \T \text{ 或者 }\chi^\sigma = \T\\
  (\psi \land \chi)^\sigma = \T           &\quad\text{iff}\quad    \psi^\sigma = \T \text{ 并且 }\chi^\sigma = \T\\
  (\psi \to \chi)^\sigma = \T             &\quad\text{iff}\quad    \text{或者 }\psi^\sigma \neq \T \text{ 或者 }\chi^\sigma = \T\\
  (\psi \leftrightarrow \chi)^\sigma = \T &\quad\text{iff}\quad    \psi^\sigma = \chi^\sigma\\
\end{aligned}$$

令 $\Omega$ 为任意 $\mathscr{L}_0$-公式集（可以是无穷集），并令 $\sigma$ 为任意真值指派。$\sigma$ **满足** $\Omega$（符号表示：$\sigma \vDash \Omega$ 当且仅当对每个 $\phi \in \Omega$，$\phi^\sigma = \T$。我们用 $\sigma \vDash \phi$ 表示 $\sigma \vDash \{\phi\}$，即 $\phi^\sigma = \T$，并用 $\sigma \nvDash \Omega$ 和 $\sigma \nvDash \phi$ 分别表示 $\sigma \vDash \Omega$ 和 $\sigma \vDash \phi$ 不成立。

对于每个真值指派 $\sigma$，都有 $\sigma \vDash \varnothing$。这是因为，既然 $\varnothing$ 是空集，那么就不存在 $\varnothing$ 中的公式 $\phi$ 使得 $\sigma \nvDash \varnothing$。

### <span class="indianred">3.3</span> 重言蕴含、重言等值与可满足性

<div class="list-underline list-bold"></div>

* 重言蕴含

  令 $\Omega$ 为任意 $\mathscr{L}_0$-公式集（可以是无穷集）并且 $\phi$ 为任意 $\mathscr{L}_0$-公式。$\Omega$ **重言蕴含** $\phi$（符号表示 $\Omega \vDash_0 \phi$）当且仅当对每一个真值指派 $\sigma$，如果 $\sigma \vDash \Omega$ 则 $\sigma \vDash \phi$。$\phi$ 是 $\Omega$ 的**重言后承**当且仅当 $\Omega \vDash_0 \phi$。

  当 $\Delta = \{\phi_1,\ldots,\phi_n\}$ 时，可以用 $\phi_1,\ldots,\phi_n \vDash_0 \phi$ 表示 $\Delta \vDash_0 \phi$，用 $\Omega,\phi_1,\ldots,\phi_n\ \vDash_0 \phi$ 表示 $\Omega \cup \Delta \vDash_0 \phi$。当 $\Delta = \varnothing$ 时，我们用 $\vDash_0 \phi$ 表示，此时，因为对于任意真值指派 $\sigma$ 都有 $\sigma \vDash \varnothing$，所以也都有 $\sigma \vDash \phi$。
...
* 可满足性
  ...
  令 $\Omega$ 为任意 $\mathscr{L}_0$-公式集（可以是无穷集），$\phi$ 为任意 $\mathscr{L}_0$-公式。$\Omega$ 是**可满足的**当且仅当存在一个真值指派 $\sigma$ 使得 $\sigma \vDash \Omega$；$\phi$ 是**可满足的**当且仅当 $\{\phi\}$ 是可满足的。$\Omega$（或 $\phi$）是**不可满足的**当且仅当它不是可满足的。

### <span class="indianred">3.4</span> 重言式、矛盾式与或然式
